...
void dispMain() {
	...
	if (--delay_switch_t == 0){
		...
		if(!tsensor && !outtempready) {
			if(++outcnt > OUTER_SENSOR_TIMEOUT) outcnt = OUTER_SENSOR_TIMEOUT;
		}
	}
	...
	if(isCorrectTemp(ft)) Dyn_Number(ft, 1, INNER_LED);	else Dyn_Code(DI_minus, DI_minus, DI_minus, DI_minus, INNER_LED);
	
	// select current data for OUTER_LED
	if(tsensor) {
		// display PRESSURE
		if(isCorrectPressure(fp)) Dyn_Number(fp, 1, OUTER_LED);
		else Dyn_Code(DI_minus, DI_minus, DI_minus, DI_code_P, OUTER_LED);
	}else {
		...
		}else Dyn_Number(outtemp, 1, OUTER_LED);
	}
	...
	if ((phistory != hh)&&(mm < 30)) {
		...
		if((OUTER_SENSOR_FAIL) || !isCorrectTemp(outtemp)) history[phistory].outT = TEMP_LIMIT*10;
		else history[phistory].outT = outtemp;
	}
}
...
void dispHistory() {
	...
	if (isCorrectTemp(history[hh].inT)) Dyn_Number(history[hh].inT, 1, INNER_LED);
	else Dyn_Code(DI_minus, DI_minus, DI_minus, DI_minus, INNER_LED);
	...
	if(tsensor) {
		if (!isCorrectPressure(history[hh].P)) Dyn_Code(DI_minus, DI_minus, DI_minus, DI_code_P, OUTER_LED);
		else Dyn_Number(history[hh].P, 1, OUTER_LED);
	} else {
		if (isCorrectTemp(history[hh].outT)) Dyn_Number(history[hh].outT, 1, OUTER_LED);
		else Dyn_Code(DI_minus, DI_minus, DI_minus, DI_code_t, OUTER_LED);
	}	
}
