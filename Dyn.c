...
//
// Необходимо предварительно задать соответствия для
// катодов и анодов в подключаемом файле Dyn_def.h
//
// В массиве 
//		unsigned char DI_Digit[4][DI_Second_Led+1]
// размещаются коды символов выводимых на индикаторы
// Функция Dyn_Indicate() вызывается из таймера
// и выполняет последовательный перебор катодов
//========================================================
#include "Dyn.h"
...
//                      Dyn_Indicate()
//                   Управление катодами
//        Выставление на индикаор кода и выбор анода
//=========================================================
void Dyn_Indicate()
{	
	...
	#endif
	#ifdef DI_LED3_K1000
		if(DI_LED3_K1000) {	//перебор разрядов всех индикаторов закончен
			DI_LED3_K1000 = 0;
			Dyn_SignOut(DI_Digit[DI_p1][DI_LED1]);
			DI_LED1_K1 = 1;
			return;
	...
	#endif
	//если вдруг недопустимое значение, начинаем сначала
...
void Dyn_Number(int value, char dot, char led)
{
	...
	for(i = digcnt; i < 5; i++)// Все неиспользуемые места в массиве заполняем пустотой
	{
		p[i] = DI_null;
	}
	...
	for(i = 0; i < digcnt; i++)// Разбиваем число на разряды
	{
		p[i] = digits[value % 10];
		value /= 10;
	}
	...
	i = digcnt - 1;
	while(i > 0)// Удаляем незначащие нули впереди
	{
		if((p[i+1] == DI_null) && (p[i] == digits[0])) p[i] = DI_null;
		i--;
	}
	
	if(value > 0)// Если число не помещается, сообщаем ошибкой
	{
		p[digcnt - 1] = DI_code_E;
	}
	
	if(sign && (p[digcnt - 1] != DI_null)) // Если знак не помещается, сообщаем ошибкой
	{
		p[digcnt - 1] = DI_code_E;
	}
